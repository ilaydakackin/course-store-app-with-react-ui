...
axios.defaults.baseURL = "http://localhost:5168/api/";
axios.defaults.withCredentials =true;

//Tokenı servise göndermek için
axios.interceptors.request.use(request => {
    const token = store.getState().account.user?.token;

    //Eğer token varsa request headerına ekle
    if(token)
    {
...
const queries = {
    ...
    post: (url: string, body:object) => axios.post(url, body).then((response: AxiosResponse) => {
...
const Catalog = {
    ...
    details: (id: number) => queries.get(`products/${id}`) as Promise<IProduct>,
}
...
const Account ={
    ...
    logout: () => queries.get("account/logout"),
...
const requests = {
    Catalog, Errors, Cart, Account, Orders
}
